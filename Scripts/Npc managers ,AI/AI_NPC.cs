...
public enum AI_STATE {IDLE_STATE,PATROL_STATE,RANDOM_WALK_STATE,BATTLE_STATE};

public class AI_NPC : MonoBehaviour {
    NPC_Human_Manager thisNPC;
    ...
    private void Awake()
    {
        ...
        thisNPC = GetComponent<NPC_Human_Manager>();
    ...
    IEnumerator RandomWalk()
    {
        ...
        while (true)
        {
            ...
            if (pointIsReachable)
            {
                thisAnimator.SetFloat("speed", 0.2f);
    ...
    IEnumerator ChaseState()
    {
        ...
        if (thisAnimator.GetFloat("speed") < 0.4f) thisAnimator.SetFloat("speed", 0.4f);
        thisAgent.isStopped = false;
    ...
    IEnumerator LookAtTarget()
    {
        ...
        if (thisAnimator.GetFloat("speed") != 0.2f) thisAnimator.SetFloat("speed", 0.2f);
        thisAgent.isStopped = true;
        ...
        while (angle > 5){
            ...
            gameObject.transform.rotation = Quaternion.Lerp(gameObject.transform.rotation, direction, 360/ angle * Time.deltaTime);
    ...
    IEnumerator AttackState()
    {
        ...
        thisAnimator.SetTrigger("AttackRight");
        yield return new WaitForSeconds(0.42f);
        
        //Начало нанесения урона
        if (weaponCollider != null && !thisNPC.IsDead())
        {
    ...
    void MakeStep()
    {
        ...
        if (aSrc != null)
        {
            if (thisNPC.stepClip != null)
            {
                aSrc.clip = thisNPC.stepClip;
                aSrc.Play();
            }
